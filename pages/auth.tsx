...
const Auth: NextPage = () => {
  ...
  const login = useCallback(async () => {
    ...
    try {
      ...
      if (!response?.ok) {
        // response?.error có thể null => fallback string
  ...
  const register = useCallback(async () => {
    ...
    try {
      ...
      // sau khi register thành công -> login luôn
      const response = await signIn("credentials", {
        email,
        password,
        redirect: false,
      ...
      if (!response?.ok) {
        throw new Error(response?.error ?? "Auto login after register failed");
      }
  ...
  }, [email, username, password, isLoading, router]);
  ...
  const googleLogin = useCallback(async () => {
    ...
    try {
      // OAuth để redirect = true (mặc định) là ổn
      await signIn("google", { callbackUrl: "/profiles" });
    } catch (error) {
      console.log("Google login error:", error);
      setIsLoading(false);
    }
    // Không set false ở đây vì OAuth thường rời trang để redirect
  }, [isLoading]);
        ...
        <div className="w-full h-full bg-black lg:bg-opacity-50">
          ...
          <div className="flex justify-center">
            <div className="bg-black bg-opacity-70 px-6 py-6 self-cnter mt-1 lg:w-2/5 lg:max-w-md rounded-md w-full sm:px-16">
              ...
              {/* Đã bỏ guest login theo yêu cầu */}
            </div>
          </div>
        </div>
      </div>
